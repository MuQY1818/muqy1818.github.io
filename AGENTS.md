...
- Theme toggle upgraded from plain text to an animated icon chip (sun/moon SVG morph, in `academic.css`); clicking reveals the new theme in a circle expanding from the pointer via `document.startViewTransition` (clip-path animation on `::view-transition-new(root)`; falls back to an instant switch on Firefox / reduced-motion). Same handler duplicated in `blog.js` and `post.js` (`initThemeToggle`).
