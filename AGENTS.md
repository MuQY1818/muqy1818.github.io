...
- Added paper grain + mouse spotlight (both in `academic.css`): `body::before` is a fixed SVG feTurbulence noise layer (grayscale, opacity .035, z-index 90, `filter: invert(1)` in dark mode) giving the paper its tooth; `#spotlight` is a fixed radial-gradient glow following the pointer (z-index 80) — warm sun-pool `rgba(255,247,222,.7)` @560px on light, `rgba(255,232,200,.075)` @640px on dark. The div is injected by `initSpotlight()` (duplicated in `blog.js` and `post.js`, rAF-throttled pointermove writing `--mx/--my`); disabled under reduced-motion and coarse pointers.
- cache-bust bumped to `?v=20260724h` (academic.css + home.js + blog.css).
